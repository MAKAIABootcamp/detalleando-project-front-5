...
const Address = () => {
  ...
  const [chosenAddress, setChosenAddress] = useState(
    // currentOrder.sendTo || userLogged?.address[0] || ""
    () => {
      if (currentOrder?.sendTo?.name) {
        return currentOrder.sendTo
      } else {
        return userLogged?.address[0] || "";
      }
 } );
          ...
          : userLogged.address &&
            userLogged.address.length > 0 && (
              <>
                <div>
                  <h4>
                    {chosenAddress.direction}, {chosenAddress.name}
                  </h4>
                </div>
                {openChoice && (
                  <div className="opening-element">
